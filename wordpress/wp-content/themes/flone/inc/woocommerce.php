...
function flone_woocommerce_template_loop_product_thumbnail(){
	...
	<div class="product-action <?php if(class_exists('YITH_WCWL')){ echo esc_attr('has_wishlist'); } else { echo esc_attr('no_wishlist'); } ?>">
	    <?php
...
function flone_wc_get_rating_html($html, $rating, $count){
	...
	<div class="<?php echo esc_attr( $wrapper_class ); ?>">
    	<?php for($i = 1; $i <= 5; $i++){
			if($i <= $rating_whole){
				echo '<i class="far fa-star yellow"></i> ';
			} else {
				if($rating_fraction){
					echo '<i class="far fa-star-half yellow"></i> ';
				} else {
