<?php
	...
	class APBDWMC_general extends AppsBDLiteModule{
		...
		function initialize() {
			...
			if(!is_admin()) {
				add_filter( 'woocommerce_currency', [ $this, '_wc_set_currency' ], 99999999 );
				add_filter( 'woocommerce_currency_symbol', [ $this, '_wc_set_currency_symbol' ], 10, 2 );
				add_filter( 'woocommerce_price_format', [ $this, '_wc_set_price_format' ], 10, 2 );
			}
			add_filter( 'woocommerce_product_get_regular_price', [ $this, '_wc_get_product_price' ], 9999, 2 );
			add_filter( 'woocommerce_product_get_sale_price', [ $this, '_wc_get_product_price' ], 9999, 2 );
			add_filter( 'woocommerce_product_get_price', [ $this, '_wc_get_product_price' ], 99999, 2 );
			
			//variation prices
			add_filter( 'woocommerce_variation_prices', [$this,'_wc_get_variation_prices'], 99999, 3 );
			
			/*Variable price*/
			add_filter( 'woocommerce_product_variation_get_price', array( $this, '_wc_get_product_variation_price' ), 99, 2 );
		...
		function OnInit() {
			...
			//add_filter( 'woocommerce_product_get_sale_price', [ $this, 'product_get_sale_price' ], 99999, 2 );
			//$this->AddGlobalJSVar("new_currency_url",$this->GetActionUrl("new"));
		}
		...
		function _wc_get_product_price($price, $product = NULL){
			if ( ! $price ) {
				return $price;
			}
